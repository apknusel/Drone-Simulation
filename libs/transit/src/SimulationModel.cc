...
SimulationModel::SimulationModel(IController& controller)
  ...
  entityFactory.AddFactory(new BatteryDecoratorFactory());
  entityFactory.AddFactory(new ChargingDecoratorFactory());
  ...
  entityFactory.AddFactory(new ParrotFactory());
}
...
IEntity* SimulationModel::createEntity(JsonObject& entity) {
  ...
  if (RechargeStation* r = dynamic_cast<RechargeStation*>(myNewEntity)) {
    rechargeStation = r;
  }
...
void SimulationModel::update(double dt) {
  for (auto& [id, entity] : entities) {
    if (BatteryDecorator* b = dynamic_cast<BatteryDecorator*>(entity)) {
      if (std::string(b->getName(), 0, 13) == "Battery Drone") {
        // checks the drones charge and only allows the drone getting added once
        if (b->getBatteryCharge() == 0 &&
            std::find(deadBatteryDrones.begin(), deadBatteryDrones.end(), b) ==
                deadBatteryDrones.end()) {
          deadBatteryDrones.push_back(b);
        }
      }
    }
    entity->update(dt);
